...
jump ::  (Int -> Int) -> Maze -> Maze
jump modifier maze = if hasExit maze then maze else Maze {step = step maze + 1, current=newCurrent, jumps=newJumps}
    where index = current maze
          move = Seq.index (jumps maze) index
          newCurrent = current maze + move
          newJumps = Seq.update index (modifier move) (jumps maze)
